...
public class ChecklistController {
    ...
    @ApiOperation(value = "체크 리스트 저장", notes = "체크리스트를 저장 한다.")
    @PostMapping()
    ...
    @ApiOperation(value = "체크리스트 삭제", notes = "<strong> itemId </strong>를 통해 준비물을 삭제 한다.")
    @DeleteMapping()
    public void deleteItem(@RequestParam("itemId") Long itemId) throws IOException {
        checklistService.deleteItem(itemId);
    }

    @ApiOperation(value = "체크리스트 조회", notes = "<strong> planId </strong>를 통해 계획별 준비물을 조회한다.")
    @GetMapping("/items")
